    ...
    for artery in arteries:
        ...
        for i in range(100):
            ...
            try:
                ...
                ax.plot(np.linspace(0,1,100),unit_to_mmHg(df_pressure[0, :]), label=f"TEST_{i}")
            except:
                continue
...
def main(result_folder: str, arteries: List[str]) -> None:
    ...
    for artery in arteries:
        for i in range(100):
            ...
            try:
                ...
                result_dict[f'{artery}_MAP'].append(unit_to_mmHg(compute_MAP(df_pressure)))
                result_dict[f'{artery}_MFV'].append(compute_MFV(df_velocity))
                result_dict[f'{artery}_area'].append(np.mean(df_area))
                result_dict[f'{artery}_PI'].append(compute_PI(df_velocity, compute_MFV(df_velocity)))
            except:
                failed += 1
    ...
    df.to_csv(result_folder / "RESULTS_pfv_100_140_tau_035.csv")
